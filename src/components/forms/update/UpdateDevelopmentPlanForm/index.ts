...
import { updateProject, updateResolution, updateApproval, updateMilestone, updateMilestoneStatus, createExtension, updateExtension, createNotification, updateNotification, createPeriod, updatePeriod, deletePeriod, deleteExtension, deleteNotification, updatePeriodStatus, createVestingExtension, updateVestingExtension, deleteVestingExtension } from "../../../../context/App/AppActions"
import { savedPopup, errorPopup } from "../../../../utils/Toast/Toast"

// Types
import { UseFormReturn } from "react-hook-form"
import { Periods, NotificationTypes, Notification } from "../../../../context/App/types"
import { Approval, VestingPeriod, Milestone } from "../../../../context/App/types"
import { OnSubmitProps, UpdateDevelopmentPlanFormProps, UpdateDevelopmentPlanFormState, UseSetDatesObjProps, DatesObj, UseExpireProjectProps, UseMilestoneExt, HandleDeleteValueProps, SetValueKeys } from "./types"

export const useUpdateDevelopmentPlanForm = (data: UpdateDevelopmentPlanFormProps['data']): UseFormReturn<UpdateDevelopmentPlanFormState> => { // Update development plan form state
  return useForm<UpdateDevelopmentPlanFormState>({
    defaultValues: {
      ...
      approval: {
        ...
        BOMA: {
          date: data.Approvals.find((obj: Approval) => obj.approvedBy === 'BOMA')?.date ?? undefined,
          uuid: data.Approvals.find((obj: Approval) => obj.approvedBy === 'BOMA')?.uuid ?? ''
        }
      },
      vesting: {
        tenYear: {
...
export const onSubmit = async (formData: OnSubmitProps['formData'], options: OnSubmitProps['options']) => { // Handle form submit
  ...
  if(result.success) {
    ...
    const response = await Promise.all([
      updateResolution({ resolution: formData.resolution.resolution, uuid: formData.resolution.uuid }),
