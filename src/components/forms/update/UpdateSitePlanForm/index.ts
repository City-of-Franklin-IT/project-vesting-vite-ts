...
export const onSubmit = async (formData: OnSubmitProps['formData'], reset: OnSubmitProps['reset'], navigate: OnSubmitProps['navigate'], token: OnSubmitProps['token']): Promise<void> => {
  const projectObj = {
...
}, [values.approval.date, values.vesting.tenYear.date, values.vesting.fifteenYear.date, values.milestones.first, values.milestones.second, values.notifications.initial.date, values.notifications.lastCall.date, values.notifications.lostVesting.date])
