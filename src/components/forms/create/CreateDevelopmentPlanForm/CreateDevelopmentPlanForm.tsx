...
import { useCreateDevelopmentPlanForm, onSubmit, useSetDatesObj, useSetDates } from '.'
...
function CreateDevelopmentPlanForm() {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useCreateDevelopmentPlanForm() // useForm

  const values = watch()

  const dates = useSetDatesObj(values)

  useSetDates(dates, { setValue }) // Set dates on BOMA approval change
        ...
        <div className={styles.body}>
          ...
          <section className="flex gap-3">
            <div className={`${ styles.inputSection } w-1/2`}>
              <div className="flex">
                <label htmlFor="resolution" className={`${ styles.label } w-1/2`} >Resolution #<span className="text-error">*</span></label>
                <input 
                  type="text"
                  id="resolution" 
                  { ...register("resolution", {
              ...
              {errors.resolution && <p className={styles.error}>{errors.resolution.message}</p>}
            </div>
              ...
              <div className="flex">
                ...
                <input 
                  ...
                  { ...register("approval.FPMC.date", {
                    ...
                    validate: value =>
                      !value || new Date(value as Date) > new Date(dates.bomaDate as Date) ? "FPMC approval date must be before BOMA approval date" : true
                  }) }
                  className={styles.input} />
              </div>
                ...
                <div className="flex">
                  ...
                  <input 
                    ...
                    { ...register("approval.BOMA.date", {
                      ...
                      validate: value => 
                        !value || new Date(value as Date) < new Date(dates.fpmcDate as Date) ? "BOMA approval date must be after FPMC approval date" : true
                    })}
                    className={styles.input} />
                </div>
              ...
              <div className="flex">
                ...
                <input 
                  ...
                  { ...register("milestones.first.date", {
                    ...
                    validate: value =>
                      !value || new Date(value as Date) > new Date(dates.secondMilestoneDate as Date) ? "First milestone must be before second milestone" : true
                  }) }
                  className={styles.input} />
              </div>
              ...
              <div className="flex">
                ...
                <input 
                  ...
                  { ...register("milestones.second.date", {
                    ...
                    validate: value =>
                      !value || new Date(value as Date) < new Date(dates.firstMilestoneDate as Date) ? "Second milestone must be after first milestone" : true
                  }) } 
                  className={styles.input} />
              </div>
